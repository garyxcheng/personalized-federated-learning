...
from FLAlgorithms.users.userperavgHFonestep import UserPerAvgHFOneStep
from FLAlgorithms.servers.serverbase import Server
from utils.model_utils import read_data, read_user_data

# Implementation adapted from serverperavg per-FedAvg Server

class PerAvgHFOneStep(Server):
    def __init__(self, dataset,datasetnumber,algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
                 local_epochs, optimizer, num_users,times, beta_HF, delta_HF, decimate=1, personal_epochs=1, device=torch.device("cpu"), validation_epochs=None):
        ...
        for i in range(total_users):
            ...
            user = UserPerAvgHFOneStep(id, train, valid, test, model, batch_size, learning_rate, beta, lamda, local_epochs, optimizer ,total_users , num_users, beta_HF, delta_HF, device=device, dataset=dataset)
    ...
    def train(self):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            if glob_iter % self.decimate == 0:
                ...
                self.evaluate_n_batches(num_batches=self.personal_epochs, epoch=glob_iter)
