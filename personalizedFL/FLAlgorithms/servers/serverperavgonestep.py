...
from FLAlgorithms.users.userperavgonestep import UserPerAvgOneStep
from FLAlgorithms.servers.serverbase import Server
from utils.model_utils import read_data, read_user_data

# Implementation for per-FedAvg Server

class PerAvgOneStep(Server):
    def __init__(self, dataset,datasetnumber,algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
        ...
        for i in range(total_users):
            ...
            user = UserPerAvgOneStep(id, train, valid, test, model, batch_size, learning_rate, beta, lamda, local_epochs, optimizer, total_users , num_users, device=device, dataset=dataset)
    ...
    def train(self):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            if glob_iter % self.decimate == 0:
                ...
                print("")
                assert self.personal_epochs == 1
                # we are lazy, so we are not passing a new parameter for this; we are just going to use personal_epochs
