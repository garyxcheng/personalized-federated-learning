...
from FLAlgorithms.users.userperridgeepochbatch import UserPerRidgeEpochBatch
from utils.model_utils import read_data, read_user_data
import numpy as np

# Implementation for the Personalized FedAvg with a ridge penalty on personalization 
# This is basically a copy of serverperavg but with the initializer modified to make its own users

class PerRidgeEpochBatch(Server):
    def __init__(self, dataset,datasetnumber,algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
                 local_epochs, optimizer, num_users, times, decimate=1, personal_epochs=1, device=torch.device("cpu"), user_ridge_penalty = None, validation_epochs=None):
        ...
        for i in range(total_users):
            ...
            user = UserPerRidgeEpochBatch(id, train, valid, test, model, batch_size, learning_rate, beta, lamda, local_epochs, optimizer, device=device, dataset=dataset, user_ridge_penalty=self.user_ridge_penalty)
            self.users.append(user)
            self.total_train_samples += user.train_samples
        print("Number of users / total users:",num_users, " / " ,total_users)
        print("Finished creating PerRidgeEpoch server.")
