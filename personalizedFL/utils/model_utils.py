...
#TODO : datasetnumber in string; should be torch.load as well
def read_data(dataset, datasetnumber=None):
    ...
    '''
    print(f"datasetnumber is {datasetnumber}")
    assert datasetnumber is not None
    if dataset == "prep_FederatedEMNIST":
        my_file = os.path.join('data',"FederatedEMNIST",'data',f'federatedemnist_preprocess_data_trial{datasetnumber}.p')
        with open(my_file, "rb") as f:
            data = pickle.load(f)
        return data[0], data[1], data[2], data[3], data[4]

    if dataset == "prep_Shakespeare":
        my_file = os.path.join('data',"Shakespeare",'data',f'shakespeare_preprocess_data_trial{datasetnumber}.p')
        with open(my_file, "rb") as f:
            data = pickle.load(f)
        return data[0], data[1], data[2], data[3], data[4]

    if dataset == "prep_Stackoverflownwp":
        # my_file = os.path.join('data',"Stackoverflownwp",'data','stackoverflownwp300users_preprocess_data.p') old version with 600 users
        # my_file = os.path.join('data',"Stackoverflownwp",'data',f'stackoverflownwp300users_preprocess_data_trial{datasetnumber}.p') #w/ datasetnumber=0 is what we did the P2 sweeps on
        my_file = os.path.join('data',"Stackoverflownwp",'data',f'stackoverflownwp_preprocess_data_trial{datasetnumber}.p')
        with open(my_file, "rb") as f:
            data = pickle.load(f)
        return data[0], data[1], data[2], data[3], data[4]
    
    if dataset == "prep_CIFAR100":
        my_file = os.path.join('data',"CIFAR100",'data',f'cifar100_cutoutresnet18preprocess_data_trial{datasetnumber}.p') #The DEPRECATED FedML version is 'cifar100_preprocess_data.p'
        with open(my_file, "rb") as f:
            data = pickle.load(f)
        return data[0], data[1], data[2], data[3], data[4]

    if dataset == "prep_Femnist":
        my_file = os.path.join('data',"Femnist",'data','femnist_preprocess_data.p')
        with open(my_file, "rb") as f:
            data = pickle.load(f)
...
def read_user_data(index,data,dataset):
    ...
    valid_data = data[3][id]
    test_data = data[4][id]
    X_train, y_train, X_valid, y_valid, X_test, y_test = train_data['x'], train_data['y'], valid_data['x'], valid_data['y'], test_data['x'], test_data['y']
    if(dataset == "Mnist"):
        assert False, "this should not be used"
        # if this does eventually get used; need to add X_valid and y_valid
        ...
        X_test = torch.Tensor(X_test).type(torch.float32)
        # y_test = torch.Tensor(y_test)
    elif (dataset == "prep_FederatedEMNIST"):
        X_train = torch.Tensor(X_train).type(torch.float32)
        # y_train = torch.Tensor(y_train)
        X_valid = torch.Tensor(X_valid).type(torch.float32)
        # y_valid = torch.Tensor(y_valid)
        X_test = torch.Tensor(X_test).type(torch.float32)
        # y_test = torch.Tensor(y_test)
    elif (dataset == "prep_Stackoverflownwp"):
        X_train = torch.Tensor(X_train).type(torch.float32)
        # y_train = torch.Tensor(y_train)
        X_valid = torch.Tensor(X_valid).type(torch.float32)
        # y_valid = torch.Tensor(y_valid)
    ...
    return id, train_data, valid_data, test_data

    #OLD VERSION with no valid_data 
    # assert len(data) == 4
    # id = data[0][index]
    # train_data = data[2][id]
    # test_data = data[3][id]
